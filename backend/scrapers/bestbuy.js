...
const run = async () => {
	...
	const cssSelector =
		'button[class="button_2Xgu4 secondary_3qojI  button_1Yg9v loadMore_3AoXT regular_cDhX6"] > span';
	...
	const gpus = [];
	$(
		'div[class="col-xs-12_1GBy8 col-sm-4_NwItf col-lg-3_2V2hX x-productListItem productLine_2N9kG"]'
	).each((index, element) => {
		var gpu = {};

		gpu.link =
			'https://www.bestbuy.ca' + $(element).find('.link_3hcyN').attr('href');

		gpu.title = $(element).find('.productItemName_3IZ3c').text();

		gpu.price = parseFloat(
		...
		gpu.img = $(element)
			.find('img[class="productItemImage_1en8J"]')
			.attr('src');
